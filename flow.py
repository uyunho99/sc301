...
RAG_CONTEXT_MAX_LENGTH = 1000

# LLM 호출 시 포함할 대화 히스토리 턴 수
...
class FlowEngine:
    ...
    def __init__(
        ...
        # 런타임 config 초기화 (config 있으면 YAML 기반, 없으면 schema.py fallback)
        ...
        # Graph transition 캐시 (Step→Transition 조회 결과)
        self._step_transitions_cache: dict[str, list[dict]] = {}

    def _init_runtime_config(self, config: dict | None) -> None:
        """런타임 config 초기화. config=None이면 schema.py 상수를 fallback으로 사용."""
        if config:
            self._guide_selection = config.get("guide_selection", {})
            self._auto_compute = config.get("auto_compute", {})
            self._skip_rules = config.get("skip_rules", {})
            self._system_managed_slots = config.get("defaults", {}).get("system_slots", [])
            self._checkitem_hints = self._build_hints_from_config(config.get("check_items", {}))
            self._lookup_tables = config.get("lookup_tables", {})

            # consultation (consultation_tone.yaml에서 로드)
            consultation = config.get("consultation", {})
            self._consultation_keywords = consultation.get("keywords", {})
            self._consultation_subject_patterns = consultation.get("_compiled_subject_patterns", {})
            scoring = consultation.get("scoring", {})
            self._consultation_score_weights = scoring.get("weights", {})
            self._consultation_score_threshold = scoring.get("threshold", 6.0)
            self._consultation_recommended_q_map = consultation.get("recommended_questions", {})
            self._consultation_tone_strategies = consultation.get("tone_strategies", {})

            # flow defaults
            flow_defaults = config.get("flow_defaults", {})
            self._stale_step_threshold = flow_defaults.get("stale_step_threshold", STALE_STEP_THRESHOLD)
            self._rag_search_k = flow_defaults.get("rag_search_k", RAG_SEARCH_K)
            self._rag_search_min_score = flow_defaults.get("rag_search_min_score", RAG_SEARCH_MIN_SCORE)
            self._rag_context_max_length = flow_defaults.get("rag_context_max_length", RAG_CONTEXT_MAX_LENGTH)
            self._history_window_size = flow_defaults.get("history_window_size", HISTORY_WINDOW_SIZE)

            # persona keywords (config에서 제공 시 클래스 속성 오버라이드)
            pk = config.get("persona_keywords", {})
            if pk:
                if pk.get("keywords"):
                    self.PERSONA_KEYWORDS = pk["keywords"]
                if pk.get("signal_rules"):
                    self.PERSONA_SIGNAL_RULES = pk["signal_rules"]
                if pk.get("p5_required_signals"):
                    self._P5_REQUIRED_SIGNALS = pk["p5_required_signals"]
                if pk.get("disambiguation_questions"):
                    # "key1,key2" 문자열 → tuple 변환
                    dq = {}
                    for key_str, question in pk["disambiguation_questions"].items():
                        parts = tuple(k.strip() for k in key_str.split(","))
                        dq[parts] = question
                    self.PERSONA_DISAMBIGUATION_QUESTIONS = dq
                if pk.get("default_disambiguation_question"):
                    self.DEFAULT_DISAMBIGUATION_QUESTION = pk["default_disambiguation_question"]
                if pk.get("ambiguity_threshold") is not None:
                    self.PERSONA_AMBIGUITY_THRESHOLD = pk["ambiguity_threshold"]

            # plugin registry 로드
            from plugins import load_default_plugins
            load_default_plugins()
        else:
            # schema.py fallback (하위 호환)
            self._guide_selection = GUIDE_SELECTION_RULES
            self._auto_compute = AUTO_COMPUTABLE_SLOTS
            self._skip_rules = CONDITIONAL_SKIP_RULES
            self._system_managed_slots = SYSTEM_MANAGED_SLOTS
            self._checkitem_hints = CHECKITEM_HINTS
            self._lookup_tables = {"region_bucket": REGION_BUCKET_MAP}
            self._consultation_keywords = CONSULTATION_KEYWORDS
            self._consultation_subject_patterns = CONSULTATION_SUBJECT_PATTERNS
            self._consultation_score_weights = CONSULTATION_SCORE_WEIGHTS
            self._consultation_score_threshold = CONSULTATION_SCORE_THRESHOLD
            self._consultation_recommended_q_map = CONSULTATION_RECOMMENDED_Q_MAP
            self._consultation_tone_strategies = CONSULTATION_TONE_STRATEGIES
            self._stale_step_threshold = STALE_STEP_THRESHOLD
            self._rag_search_k = RAG_SEARCH_K
            self._rag_search_min_score = RAG_SEARCH_MIN_SCORE
            self._rag_context_max_length = RAG_CONTEXT_MAX_LENGTH
            self._history_window_size = HISTORY_WINDOW_SIZE

    @staticmethod
    def _build_hints_from_config(check_items: dict) -> dict[str, str]:
        """check_items config에서 hint 딕셔너리 빌드."""
        hints: dict[str, str] = {}
    ...
    def clear_cache(self) -> None:
        ...
        self._step_transitions_cache.clear()
    ...
    def auto_compute_slots(self, state: ConversationState) -> list[str]:
        ...
        for slot_name, rule in self._auto_compute.items():
            # 이미 값이 있으면 스킵
            ...
            # 계산 함수 호출: plugin registry 우선, instance method fallback
            fn_name = rule["compute"]
            value = None
            try:
                from plugins import COMPUTE_REGISTRY
                if fn_name in COMPUTE_REGISTRY:
                    context = {"lookup_tables": self._lookup_tables}
                    value = COMPUTE_REGISTRY[fn_name](state, context)
            except ImportError:
                pass
            if value is None:
                # instance method fallback (하위 호환)
                compute_fn = getattr(self, "_%s" % fn_name, None)
                if compute_fn:
                    value = compute_fn(state)
            if value is not None:
                state.set_slot(slot_name, value)
                computed.append(slot_name)
                logger.info("Auto-computed %s = %s", slot_name, value)
    ...
    def _compute_region_bucket(self, state: ConversationState) -> str | None:
        ...
        region_map = self._lookup_tables.get("region_bucket", {})
    ...
    def should_skip_check_item(
        ...
        if var_name in self._system_managed_slots:
            return True

        # 조건부 스킵 규칙 확인
        if var_name in self._skip_rules:
            rule = self._skip_rules[var_name]
    ...
    def next_step(self, state: ConversationState) -> TransitionResult:
        ...
        # 2. 분기 확인: Graph transition 우선, BRANCHING_RULES fallback
        graph_transitions = self._get_step_transitions(step_id)
        if graph_transitions:
            result = self._evaluate_branching_from_graph(graph_transitions, state)
            if result:
                return result
            logger.warning(
                "Graph 분기 규칙 매칭 실패, TO 관계로 fallback: step=%s", step_id
            )
        elif step_id in BRANCHING_RULES:
            result = self._evaluate_branching_rules(step_id, state)
            if result:
                return result
            logger.warning(
                "분기 규칙 매칭 실패, TO 관계로 fallback: step=%s", step_id
            )
    ...
    def _are_step_checks_filled(self, step_id: str, state: ConversationState) -> bool:
        ...
        for ci in checks:
            ...
            if var_name in self._auto_compute:
                self.auto_compute_slots(state)
                if state.is_slot_filled(var_name):
                    continue
    ...
    def _get_step_transitions(self, step_id: str) -> list[dict]:
        """Graph에서 Step→Transition→Step 조회 (캐시 적용).

        Returns:
            Transition 레코드 리스트 (priority DESC). 빈 리스트면 분기점 아님.
            각 레코드: transitionId, priority, isDefault, protocolMode,
                       targetStepId, ruleId, logic
        """
        if step_id in self._step_transitions_cache:
            return self._step_transitions_cache[step_id]

        transitions: list[dict] = []
        try:
            with self.driver.session() as session:
                result = session.run(QUERY_STEP_TRANSITIONS, stepId=step_id)
                transitions = [dict(r) for r in result]
        except Exception as e:
            logger.debug("Graph transition 조회 실패 (step=%s): %s", step_id, e)

        self._step_transitions_cache[step_id] = transitions
        return transitions
    ...
    def _evaluate_branching_from_graph(
        self,
        transitions: list[dict],
        state: ConversationState,
    ) -> "TransitionResult | None":
        """Graph에서 읽은 Transition 목록으로 분기 평가.

        transitions는 QUERY_STEP_TRANSITIONS 결과 (priority DESC 정렬 완료).
        각 레코드: transitionId, priority, isDefault, protocolMode,
                   targetStepId, ruleId, logic
        """
        ...
        for t in transitions:
            ...
            rule_id = t.get("ruleId")
            if rule_id:
                logic = t.get("logic", "AND")
                if self._evaluate_rule_filtered(rule_id, state, logic=logic):
                    protocol = t.get("protocolMode") or None
                    return TransitionResult(
                        t["targetStepId"],
                        "branching",
                        {"transitionId": t["transitionId"], "ruleId": rule_id},
                        protocol_mode=protocol,
                    )
            else:
                # ruleId 없는 조건부 transition (있을 수 없지만 방어)
                pass

        if default_transition:
            protocol = default_transition.get("protocolMode") or None
            return TransitionResult(
                default_transition["targetStepId"],
                "branching",
                {"transitionId": default_transition["transitionId"], "isDefault": True},
                protocol_mode=protocol,
            )
    ...
    def _evaluate_rule_filtered(
        self, rule_id: str, state: ConversationState, *, logic: str | None = None,
    ) -> bool:
        """
        ...
        Args:
            logic: Graph에서 읽은 AND/OR 로직. None이면 OR_LOGIC_RULES 참조.
        """
        conditions = None
        ...
        # AND/OR 로직 결정 (graph에서 전달된 logic 우선, 없으면 OR_LOGIC_RULES 참조)
        results = [self._evaluate_condition(c, state) for c in conditions]

        use_or = (logic == "OR") if logic else (rule_id in OR_LOGIC_RULES)
        if use_or:
            return any(results)
        return all(results)
    ...
    def select_guides(self, step_id: str, state: ConversationState, all_guides: list[dict]) -> list[dict]:
        ...
        # guide_selection에 해당 Step이 있으면 조건부 선택
        if step_id in self._guide_selection:
            rule = self._guide_selection[step_id]
            condition_var = rule.get("condition_var") or rule.get("conditionVar")
    ...
    def _build_variable_desc(self, ci: dict, state: ConversationState) -> str | None:
        ...
        hint = self._checkitem_hints.get(var_name, "")
        if hint:
    ...
    def extract_slots(
        ...
        # - 분기점이면 다음 스텝이 불확정이므로 스킵
        # - TO 관계가 1개인 경우만 (다중 경로면 스킵)
        prefetch_var_names = set()
        is_branching = bool(self._get_step_transitions(current_step)) or current_step in BRANCHING_RULES
        if not is_branching:
...
JSON 반환: {{"변수명": "값"}}"""
    ...
    def score_consultation_persona(
        ...
        # threshold 체크 → 확정
        threshold = self._consultation_score_threshold
        top_persona = max(
            state.consultation_scores,
    ...
    def _rule_score_consultation(self, user_text: str) -> dict[str, float]:
        ...
        text_lower = user_text.lower()
        kw_weight = self._consultation_score_weights.get("keyword_match", 1.0)
        subj_weight = self._consultation_score_weights.get("subject_pattern", 2.0)

        for persona, keywords in self._consultation_keywords.items():
            kw_count = sum(1 for kw in keywords if kw in text_lower)
            scores[persona] = kw_count * kw_weight

        for persona, patterns in self._consultation_subject_patterns.items():
    ...
    def _llm_score_consultation(
        ...
        multiplier = self._consultation_score_weights.get("llm_score_multiplier", 1.5)
...
"""
    ...
    def _check_recommended_q_match(self, user_text: str) -> dict[str, float]:
        ...
        scores: dict[str, float] = {}
        rq_weight = self._consultation_score_weights.get("recommended_q", 3.0)
        for question, persona in self._consultation_recommended_q_map.items():
    ...
    def _build_persona_context(self, state: ConversationState) -> str:
        ...
        # 상담 Persona 톤 주입
        if state.consultation_persona:
            tone = self._consultation_tone_strategies.get(state.consultation_persona)
            if tone:
                parts.append("")
    ...
    def _build_collect_prompt(
        ...
        display_filled = {k: v for k, v in filled.items() if k not in self._system_managed_slots}
...
- 시술 방법, 효과, 부작용에 대한 설명은 질문이 있을 때만 답하세요."""
    ...
    def _collect_rag_context(self, chunks: list) -> str:
        """RAG 검색 결과를 RAG_CONTEXT_MAX_LENGTH 이내로 결합."""
        context_parts = []
        total_length = 0
        for chunk in chunks:
            if total_length + len(chunk.content) > self._rag_context_max_length:
                break
            context_parts.append(chunk.content)
            total_length += len(chunk.content)
        return "\n".join(context_parts)

    def _do_rag_search_sync(self, core, user_text: str) -> str:
        """동기 RAG 검색. core가 None이면 빈 문자열 반환."""
        if not core:
            return ""
        try:
            chunks = core.vector_search_combined(
                user_text, k=self._rag_search_k, min_score=self._rag_search_min_score
            )
            return self._collect_rag_context(chunks)
        except Exception as e:
    ...
    def _check_early_return(self, state: ConversationState) -> str | None:
        """disambiguation 또는 no-step 상태에서 조기 반환 메시지. None이면 계속 진행."""
        if state.persona_disambiguation is not None:
            return state.persona_disambiguation["question"]
        if not state.current_step_id:
            return "적절한 상담 시나리오를 찾지 못했습니다. 어떤 상담이 필요하신가요?"
        return None

    def _advance_state(self, state: ConversationState, user_text: str) -> str:
        """
        Slot 추출 후 상태 진행: auto compute → scoring → stale check → transition → chain.
        ...
        if turns_at_step >= self._stale_step_threshold:
            logger.warning(
    ...
    def _build_final_prompt(
        ...
        """현재 스텝의 system prompt 생성. step 없으면 None."""
        if not state.current_step_id:
            return None
    ...
    def process_turn(
        ...
        early = self._check_early_return(state)
        if early:
            state.add_turn("assistant", early)
            return early, state

        # Slot 추출 || RAG 검색 병렬 실행 (전략 15)
        with ThreadPoolExecutor(max_workers=2) as executor:
            rag_future = executor.submit(self._do_rag_search_sync, core, user_text)
            state = self.extract_slots(state, user_text)
            rag_context = rag_future.result()

        inform_context = self._advance_state(state, user_text)
        system_prompt = self._build_final_prompt(state, rag_context, inform_context)

        if system_prompt:
            if self.openai:
                history = state.get_history_as_messages(n=self._history_window_size)
    ...
    def _generate_response(self, system_prompt: str, history: list[dict]) -> str:
        """LLM 응답 생성"""
        messages = self._build_llm_messages(system_prompt, history)
        response = self.openai.chat.completions.create(
    ...
    def process_turn_streaming(
        ...
        if system_prompt and self.openai:
            history = state.get_history_as_messages(n=self._history_window_size)
            full_response = ""
            ...
            response = f"[{step.step_type}] {step.desc}" if step else "상담이 완료되었습니다. 추가 질문이 있으시면 말씀해 주세요."
            yield response
    ...
    async def process_turn_async(
        ...
        if core and hasattr(core, 'vector_search_combined_async'):
            try:
                ...
                rag_context = self._collect_rag_context(chunks)
            except Exception as e:
                logger.warning(f"RAG 검색 실패: {e}")
